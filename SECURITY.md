...
In scope: the twelve plugin binaries this repository releases, the manifests under `index/` (the index rta installs them from), and the release pipeline that produces both. rta's own verification of a plugin — digest checking, declaration checking, confinement — is [rta](https://github.com/this-is-tobi/rta)'s scope; report those there.
