...
func TestRestoreRefusesMCP(t *testing.T) {
	r := req(t, "mysql.restore", map[string]any{
...
func TestRestoreRequiresADatabase(t *testing.T) {
	_, err := runRestore(context.Background(), req(t, "mysql.restore", map[string]any{
		"file": dumpOnDisk(t, "sql"),
	}))
	var verr *view.Error
	if !errors.As(err, &verr) || verr.Code != "mysql.restore.nodatabase" {
		t.Fatalf("err = %v, want mysql.restore.nodatabase", err)
	}
}
...
func TestRestoreRefusesAMissingFile(t *testing.T) {
	...
	var verr *view.Error
	if !errors.As(err, &verr) || verr.Code != "mysql.restore.nofile" {
		t.Fatalf("err = %v, want mysql.restore.nofile", err)
	}
}
...
func TestRestoreRefusesAnEmptyFile(t *testing.T) {
	...
	var verr *view.Error
	if !errors.As(err, &verr) || verr.Code != "mysql.restore.empty" {
		t.Fatalf("err = %v, want mysql.restore.empty", err)
	}
}
...
func TestRestoreArgsCarryTheDecidedFlags(t *testing.T) {
	args := strings.Join(restoreArgs(req(t, "mysql.restore", map[string]any{
...
func TestClassifyRestoreNamesTheFailure(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"no target": {"ERROR 1049 (42000): Unknown database 'app'", "mysql.restore.notarget"},
		"auth":      {"ERROR 1045 (28000): Access denied for user 'app'@'%'", "mysql.auth.failed"},
		"read only": {"ERROR 1290 (HY000): The MySQL server is running with the --read-only option", "mysql.restore.readonly"},
...
func TestAMidFileFailureNamesItsLineAndThePartialState(t *testing.T) {
	verr := classifyRestore(errors.New("exit status 1"),
		...
		req(t, "mysql.restore", map[string]any{"database": "app"}))
