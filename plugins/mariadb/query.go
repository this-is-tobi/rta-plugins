...
func queryView(ctx context.Context, db *sql.DB, req plugin.Request) (view.View, error) {
	...
	case errors.Is(err, ErrTooLarge):
		return nil, view.Errorf("mariadb.query.toolarge",
...
func activityCapability() plugin.Capability {
	return cap(plugin.Capability{
		...
		//
		// mariadb.overview --detail keeps the same rows without that column —
...
// activityView is shared by mariadb.activity and mariadb.overview --detail, and
